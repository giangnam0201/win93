...
class WaterNode extends TempNode {
  constructor(options, waterBody) {
    ...
    this.flowMap = texture(
      options.flowMap !== undefined ? options.flowMap : null,
    )
  ...
  setup() {
    const outputNode = Fn(() => {
      ...
      const refractionSampler = viewportSharedTexture(
        viewportSafeUV(refractorUV),
      )

      // calculate final uv coords

      return vec4(this.color, 1.0).mul(
        mix(refractionSampler, reflectionSampler, reflectance),
      )
    })()

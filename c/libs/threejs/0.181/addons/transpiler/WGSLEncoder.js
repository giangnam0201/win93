  ...
  emitTextureAccess(node) {
    ...
    switch (node.name) {
      ...
      case "texelFetch":
        ...
        const lodFetch =
          node.params.length > 2 ? this.emitExpression(node.params[2]) : "0"
  ...
  emitSwitch(node) {
    ...
    for (const switchCase of node.cases) {
      ...
      if (switchCase.isDefault) {
        ...
        const cases = switchCase.conditions
          .map((c) => this.emitExpression(c))
          .join(", ")
  ...
  emitFunction(node) {
    ...
    for (const param of node.params) {
      ...
      if (param.linker && param.linker.assignments.length > 0) {
        ...
        const mutableVar = new VariableDeclaration(
          param.type,
          param.name,
          immutableAccessor,
        )
      ...
      for (const uniform of this.uniforms) {
        ...
        if (uniform.type.includes("texture")) {
          ...
          uniformStructMembers.push(
            `\t${uniform.name}: ${this.getWgslType(uniform.type)},`,
          )
        }
      }

...
${this.tab}} )`
  ...
  emitLoop(node) {
    ...
    if (node.afterthought.isUnary) {
      ...
      if (
        node.afterthought.right.isAccessor ||
        node.afterthought.right.isNumber
      ) {
  ...
  emitFor(node) {
    ...
    if (
      ...
      ((afterthought.isUnary &&
        ...
        (afterthought.isOperator &&
          initialization.name === afterthought.left.property))
    ) {
...
${this.tab}}`
  ...
  emitComment(statement, body) {
    ...
    output +=
      this.tab + statement.comment.replace(/\n/g, "\n" + this.tab) + "\n"
  ...
  emit(ast) {
    ...
    if (this.iife) {
      ...
      header +=
        imports.length > 0
          ? "\tconst { " + imports.join(", ") + " } = TSL;\n"
          : ""
      footer +=
        exports.length > 0 ? "\treturn { " + exports.join(", ") + " };\n" : ""

      footer += "\n} );"
    } else {
      header +=
        imports.length > 0
          ? "import { " + imports.join(", ") + " } from 'three/tsl';\n"
          : ""
    }

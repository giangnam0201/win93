...
class GLSLDecoder {
  ...
  parseExpressionFromTokens(tokens) {
    ...
    for (const operators of precedenceOperators) {
      const parseToken = (i, inverse = false) => {
        ...
        if (groupIndex === 0 && operators.includes(token.str)) {
          ...
          if (operator === "?") {
            ...
            const leftTokens = this.getTokensUntil(":", tokens, i + 1).slice(
              0,
              -1,
            )
            ...
            const right = this.parseExpressionFromTokens(
      ...
      const isRightAssociative = operators.some((op) =>
        associativityRightToLeft.includes(op),
      )
    ...
    if (lastToken.isOperator) {
      for (const operator of unaryOperators) {
        if (lastToken.str === operator) {
          const left = this.parseExpressionFromTokens(
    ...
    if (firstToken.str === "(") {
      ...
      const left = this.parseExpressionFromTokens(
        leftTokens.slice(1, leftTokens.length - 1),
      )
    ...
    if (firstToken.isNumber) {
      ...
      if (secondToken) {
        if (secondToken.str === "(") {
          ...
          const internalTokens = this.getTokensUntil(")", tokens, 1).slice(
            1,
            -1,
          )

          const paramsTokens =
            this.parseFunctionParametersFromTokens(internalTokens)

          const functionCall = new FunctionCall(
            getFunctionName(firstToken.str),
            paramsTokens,
          )
  ...
  parseExpression() {
    ...
    const exp = this.parseExpressionFromTokens(
      tokens.slice(0, tokens.length - 1),
    )
  ...
  parseFunction() {
    ...
    const params = this.parseFunctionParams(
      paramsTokens.slice(1, paramsTokens.length - 1),
    )
  ...
  parseVariablesFromToken(tokens, type) {
    ...
    if (token) {
      ...
      if (initTokens[0].str === "=") {
        ...
        if (expressionTokens[expressionTokens.length - 1].str === ",")
  ...
  parseFor() {
    ...
    const initializationTokens = this.getTokensUntil(";", forTokens, 0).slice(
      0,
      -1,
    )
  ...
  parseSwitch() {
    ...
    const discriminant = this.parseExpressionFromTokens(
      switchDeterminantTokens.slice(1, -1),
    )
  ...
  parseSwitchCases() {
    ...
    while (isCase(token)) {
      ...
      if (token.str === "case") {
        ...
        const caseStatement = this.parseExpressionFromTokens(
          caseTokens.slice(0, -1),
        )
  ...
  parseIf() {
    const parseIfExpression = () => {
      ...
      return this.parseExpressionFromTokens(
        condTokens.slice(1, condTokens.length - 1),
      )
    }
  ...
  parseBlock() {
    ...
    while (this.index < this.tokens.length) {
      ...
      if (
        groupIndex === 0 &&
        (token.str === "case" || token.str === "default")
      ) {

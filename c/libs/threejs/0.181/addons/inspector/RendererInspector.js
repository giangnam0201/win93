...
export class RendererInspector extends InspectorBase {
  ...
  finish() {
    ...
    frame.deltaTime =
      now - (this._lastFinishTime > 0 ? this._lastFinishTime : now)
  ...
  async resolveTimestamp() {
    ...
    this._resolveTimestampPromise = new Promise((resolve) => {
      requestAnimationFrame(async () => {
        ...
        const computeFrames = renderer.backend.getTimestampFrames(
          TimestampQuery.COMPUTE,
        )
        const renderFrames = renderer.backend.getTimestampFrames(
  ...
  finishCompute() {
    ...
    this.currentCompute = currentCompute.parent.isComputeStats
      ? currentCompute.parent
      : null
  }

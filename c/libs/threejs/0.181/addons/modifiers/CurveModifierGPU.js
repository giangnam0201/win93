...
function modifyShader(material, uniforms, numberOfCurves) {
  ...
  material.positionNode = Fn(() => {
    ...
    const spinePortion = select(
      bend,
      worldPos.x.add(spineOffset).div(spineLength),
      0,
    )
    const mt = spinePortion
      .mul(pathSegment)
      .add(pathOffset)
      .mul(textureStacks)
      .toVar()
    ...
    const spinePos = texture(
      spineTexture,
      vec2(mt, rowOffset.add(0.5).div(textureScale)),
    ).xyz

    const a = texture(
      spineTexture,
      vec2(mt, rowOffset.add(1.5).div(textureScale)),
    ).xyz
    const b = texture(
      spineTexture,
      vec2(mt, rowOffset.add(2.5).div(textureScale)),
    ).xyz
    const c = texture(
      spineTexture,
      vec2(mt, rowOffset.add(3.5).div(textureScale)),
    ).xyz
    ...
    return basis
      .mul(vec3(worldPos.x.mul(xWeight), worldPos.y, worldPos.z))
      .add(spinePos)
  })()

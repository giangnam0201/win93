...
const softLightMix = TSL.Fn(([t, col1, col2]) => {
  ...
  return tm
    .mul(col1)
    .add(t.mul(one.sub(col1).mul(col2).mul(col1).add(col1.mul(scr))))
})
...
const noiseFbm3d = TSL.Fn(
  ([p, detail, roughness, lacunarity, useNormalize]) => {
    const fscale = TSL.float(1.0).toVar()
...
const woodRings = TSL.Fn(
  ([
    ...
    const blurredRings = TSL.smoothstep(
      blurAmount.negate(),
      blurAmount,
      sharpRings.sub(0.5),
    )
...
const woodDetail = TSL.Fn(([warp, p, y, splotchScale]) => {
  const radialCoords = TSL.clamp(
    TSL.atan(warp.y, warp.x).div(TSL.PI2).add(0.5),
    0,
    1,
  ).mul(TSL.PI2.mul(3))
  const combinedXyz = TSL.vec3(
    radialCoords.sin(),
    y,
    radialCoords.cos().mul(p.z),
  )
...
const cellStructure = TSL.Fn(([p, cellScale, cellSize]) => {
  const warp = spaceWarp(
    p.mul(cellScale.div(50)),
    cellScale.div(1000),
    0.1,
    1.77,
  )
...
const wood = TSL.Fn(
  ([
    ...
    return softLightMix(
      splotchIntensity,
      softLightMix(0.407, baseColor, cells),
      detail,
    )
  },
)
...
uniforms.largeGrainStretch = TSL.uniform(
  params.largeGrainStretch,
).onObjectUpdate(({ material }) => material.largeGrainStretch)
uniforms.smallWarpStrength = TSL.uniform(
  params.smallWarpStrength,
).onObjectUpdate(({ material }) => material.smallWarpStrength)
...
uniforms.ringBias = TSL.uniform(params.ringBias).onObjectUpdate(
  ({ material }) => material.ringBias,
)
uniforms.ringSizeVariance = TSL.uniform(params.ringSizeVariance).onObjectUpdate(
  ({ material }) => material.ringSizeVariance,
)
uniforms.ringVarianceScale = TSL.uniform(
  params.ringVarianceScale,
).onObjectUpdate(({ material }) => material.ringVarianceScale)
...
uniforms.cellSize = TSL.uniform(params.cellSize).onObjectUpdate(
  ({ material }) => material.cellSize,
)
uniforms.darkGrainColor = TSL.uniform(
  new THREE.Color(params.darkGrainColor),
).onObjectUpdate(({ material }, self) =>
  self.value.set(material.darkGrainColor),
)
uniforms.lightGrainColor = TSL.uniform(
  new THREE.Color(params.lightGrainColor),
).onObjectUpdate(({ material }, self) =>

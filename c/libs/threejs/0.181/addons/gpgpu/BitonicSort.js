 ...
 */
export const getBitonicFlipIndices = /*@__PURE__*/ Fn(
  ([index, blockHeight]) => {
 ...
 * @returns {Node<uvec2>} The indices of the elements in the data buffer being compared.
 */
export const getBitonicDisperseIndices = /*@__PURE__*/ Fn(
  ([index, swapSpan]) => {
...
export class BitonicSort {
  ...
  constructor(renderer, dataBuffer, options = {}) {
     ...
     */
    this.localStorage = workgroupArray(
      dataBuffer.nodeType,
     ...
     * @type {StorageBufferNode}
     */
    this.tempBuffer = instancedArray(this.count, dataBuffer.nodeType).setName(
      "TempStorage",
    )
     ...
     * @type {StorageBufferNode}
     */
    this.infoStorage = instancedArray(
      new Uint32Array([1, 2, 2]),
      "uint",
    ).setName("BitonicSortInfo")
  ...
  _getSwapLocal() {
    ...
    const fnDef = Fn(() => {
      ...
      localStorage
        .element(localID1)
        .assign(dataBuffer.element(localOffset.add(localID1)))
      localStorage
        .element(localID2)
        .assign(dataBuffer.element(localOffset.add(localID2)))
      ...
      Loop(
        ...
        () => {
          ...
          const flipIdx = getBitonicFlipIndices(
            invocationLocalIndex,
            flipBlockHeight,
          )
          ...
          Loop(
            ...
            () => {
              ...
              const disperseIdx = getBitonicDisperseIndices(
                invocationLocalIndex,
                localBlockHeight,
              )
  ...
  _getDisperseLocal(readWriteBuffer) {
    ...
    const fnDef = Fn(() => {
      ...
      localStorage
        .element(localID1)
        .assign(readWriteBuffer.element(localOffset.add(localID1)))
      localStorage
        .element(localID2)
        .assign(readWriteBuffer.element(localOffset.add(localID2)))
  ...
  _getAlignFn() {
    ...
    const fnDef = Fn(() => {
      dataBuffer
        ...
        .assign(tempBuffer.element(instanceIndex))
  ...
  computeStep(renderer) {
    ...
    if (this.currentDispatch === 0) {
      ...
      const swapType =
        this.globalOpsRemaining === this.globalOpsInSpan ? "Flip" : "Disperse"

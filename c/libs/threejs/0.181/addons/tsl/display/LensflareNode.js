...
class LensflareNode extends TempNode {
  ...
  setup(builder) {
    const lensflare = Fn(() => {
      ...
      const ghostVec = sub(vec2(0.5), texCoord)
        .mul(this.ghostSpacingNode)
        .toVar()
      ...
      Loop(
        ...
        ({ i }) => {
          ...
          sample = max(sample.sub(this.thresholdNode), vec3(0)).mul(
            this.ghostTintNode,
          )
    ...
    this._material.fragmentNode = lensflare().context(
...
export const lensflare = (node, params) =>
  nodeObject(new LensflareNode(convertToTexture(node), params))

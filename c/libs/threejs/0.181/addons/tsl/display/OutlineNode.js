...
class OutlineNode extends TempNode {
  ...
  constructor(scene, camera, params = {}) {
     ...
     * @type {TextureNode}
     */
    this._depthTextureUniform = texture(
      this._renderTargetDepthBuffer.depthTexture,
    )
     ...
     * @type {TextureNode}
     */
    this._maskTextureDownsSampleUniform = texture(
      this._renderTargetMaskDownSampleBuffer.texture,
    )
     ...
     * @type {TextureNode}
     */
    this._blurColorTextureUniform = texture(
      this._renderTargetEdgeBuffer1.texture,
    )
  ...
  updateBefore(frame) {
    ...
    _rendererState = RendererUtils.resetRendererAndSceneState(
      renderer,
      scene,
      _rendererState,
    )
  ...
  setup() {
    ...
    const prepareMask = () => {
      ...
      if (this.camera.isPerspectiveCamera) {
        viewZNode = perspectiveDepthToViewZ(
          depth,
          this._cameraNear,
          this._cameraFar,
        )
      } else {
        viewZNode = orthographicDepthToViewZ(
          depth,
          this._cameraNear,
          this._cameraFar,
        )
    ...
    const edgeDetection = Fn(() => {
      ...
      const uvNode = uv()
      const c1 = this._maskTextureDownsSampleUniform
        .sample(uvNode.add(uvOffset.xy))
        .toVar()
      const c2 = this._maskTextureDownsSampleUniform
        .sample(uvNode.sub(uvOffset.xy))
        .toVar()
      const c3 = this._maskTextureDownsSampleUniform
        .sample(uvNode.add(uvOffset.yw))
        .toVar()
      const c4 = this._maskTextureDownsSampleUniform
        .sample(uvNode.sub(uvOffset.yw))
        .toVar()
    ...
    const separableBlur = Fn(([kernelRadius]) => {
      ...
      const diffuseSum = this._blurColorTextureUniform
        .sample(uvNode)
        .mul(weightSum)
        .toVar()
      const delta = this._blurDirection
        .mul(invSize)
        .mul(kernelRadius)
        .div(MAX_RADIUS)
        .toVar()
      ...
      Loop(
        ...
        ({ i }) => {
          ...
          const sample1 = this._blurColorTextureUniform.sample(
            uvNode.add(uvOffset),
          )
          const sample2 = this._blurColorTextureUniform.sample(
            uvNode.sub(uvOffset),
          )
    ...
    this._separableBlurMaterial.fragmentNode = separableBlur(
      this.edgeThicknessNode,
    )
...
export const outline = (scene, camera, params) =>
  nodeObject(new OutlineNode(scene, camera, params))

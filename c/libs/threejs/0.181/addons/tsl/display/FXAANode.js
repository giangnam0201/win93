...
class FXAANode extends TempNode {
  ...
  setup(/* builder */) {
    ...
    const ShouldSkipPixel = (l) => {
      const threshold = max(
        _ContrastThreshold,
        _RelativeThreshold.mul(l.highest),
      )
    ...
    const DetermineEdgeBlendFactor = (texSize, l, e, uv) => {
      ...
      const pAtEnd = abs(pLuminanceDelta)
        .greaterThanEqual(gradientThreshold)
        .toVar()
      ...
      const nAtEnd = abs(nLuminanceDelta)
        .greaterThanEqual(gradientThreshold)
        .toVar()
      ...
      If(deltaSign.equal(l.m.sub(edgeLuminance).greaterThanEqual(0.0)), () => {
        ...
        blendFactor.assign(
          float(0.5).sub(shortestDistance.div(pDistance.add(nDistance))),
        )
      })

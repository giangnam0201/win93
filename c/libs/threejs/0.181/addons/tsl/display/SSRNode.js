...
class SSRNode extends TempNode {
  ...
  constructor(
    ...
    if (camera === null) {
      if (
        this.colorNode.passNode &&
        this.colorNode.passNode.isPassNode === true
      ) {
    ...
    if (this.roughnessNode !== null) {
      ...
      blurredTextureNode = passTexture(
        this,
        this._blurRenderTarget.texture,
      ).level(lod)
    }
   ...
   */
  getTextureNode() {
    return this.roughnessNode !== null
      ? this._blurredTextureNode
      : this._textureNode
  }
  ...
  setup(builder) {
    ...
    const ssr = Fn(() => {
      ...
      const d1viewPosition = viewPosition
        .add(viewReflectDir.mul(maxReflectRayLen))
        .toVar()

      // check if d1viewPosition lies behind the camera near plane
      If(
        this._isPerspectiveCamera.and(
          d1viewPosition.z.greaterThan(this._cameraNear.negate()),
        ),
        () => {
          // if so, ensure d1viewPosition is clamped on the near plane.
          // this prevents artifacts during the ray marching process
          const t = sub(this._cameraNear.negate(), viewPosition.z).div(
            viewReflectDir.z,
          )
      ...
      const totalStep = int(
        max(abs(xLen), abs(yLen)).mul(this.quality.clamp()),
      ).toConst()
      ...
      Loop(totalStep, ({ i }) => {
        ...
        const xy = vec2(
          d0.x.add(xSpan.mul(float(i))),
          d0.y.add(ySpan.mul(float(i))),
        ).toVar()
        ...
        If(this._isPerspectiveCamera, () => {
          ...
          viewReflectRayZ.assign(
            float(1).div(
              recipVPZ.add(s.mul(float(1).div(d1viewPosition.z).sub(recipVPZ))),
            ),
          )
        }).Else(() => {
          viewReflectRayZ.assign(
            viewPosition.z.add(s.mul(d1viewPosition.z.sub(viewPosition.z))),
          )
        })
        ...
        If(viewReflectRayZ.lessThanEqual(vZ), () => {
          ...
          const vP = getViewPosition(
            uvNode,
            d,
            this._cameraProjectionMatrixInverse,
          ).toVar()
          const away = pointToLineDistance(
            vP,
            viewPosition,
            d1viewPosition,
          ).toVar()
          ...
          If(away.lessThanEqual(tk), () => {
            ...
            const distance = pointPlaneDistance(
              vP,
              viewPosition,
              viewNormal,
            ).toVar()
            ...
            const fresnelCoe = div(
              dot(viewIncidentDir, viewReflectDir).add(1),
              2,
            )

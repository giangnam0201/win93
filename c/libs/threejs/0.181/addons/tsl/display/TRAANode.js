...
class TRAANode extends TempNode {
  ...
  updateBefore(frame) {
    ...
    if (
      ...
      renderer.copyTextureToTexture(
        currentDepth,
        this._historyRenderTarget.depthTexture,
      )
  ...
  setup(builder) {
    ...
    const resolve = Fn(() => {
      ...
      Loop(
        ...
        ({ x }) => {
          Loop(
            ...
            ({ y }) => {
              const uvNeighbor = uvNode
                .add(vec2(float(x), float(y)).mul(this._invSize))
                .toVar()
              const colorNeighbor = max(
                vec4(0),
                sampleTexture.sample(uvNeighbor),
              ).toVar() // use max() to avoid propagate garbage values
      ...
      const offset = velocityTexture
        .sample(closestDepthPixelPosition)
        .xy.mul(vec2(0.5, -0.5)) // NDC to uv offset
      ...
      const currentWorldPosition = this._cameraWorldMatrix.mul(
        vec4(currentViewPosition, 1.0),
      ).xyz
      ...
      const previousViewPosition = getViewPosition(
        historyUV,
        previousDepth,
        this._previousCameraProjectionMatrixInverse,
      )
      const previousWorldPosition = this._previousCameraWorldMatrix.mul(
        vec4(previousViewPosition, 1.0),
      ).xyz

      // calculate difference in world positions

      const worldPositionDifference = length(
        currentWorldPosition.sub(previousWorldPosition),
      ).toVar()
      worldPositionDifference.assign(
        min(max(worldPositionDifference.sub(1.0), 0.0), 1.0),
      )
      ...
      const motionFactor = max(
        worldPositionDifference.mul(0.5),
        velocityMagnitude.mul(10.0),
      ).toVar()
      ...
      const strongDisocclusion = worldPositionDifference
        .greaterThan(0.5)
        .and(isEdge.not())
      ...
      const compressedCurrent = currentColor.mul(
        float(1).div(
          max(currentColor.r, currentColor.g, currentColor.b).add(1.0),
        ),
      )
      const compressedHistory = clampedHistoryColor.mul(
        float(1).div(
          max(
            clampedHistoryColor.r,
            clampedHistoryColor.g,
            clampedHistoryColor.b,
          ).add(1.0),
        ),
      )
      ...
      const smoothedOutput = add(
        currentColor.mul(currentWeight),
        clampedHistoryColor.mul(historyWeight),
      )

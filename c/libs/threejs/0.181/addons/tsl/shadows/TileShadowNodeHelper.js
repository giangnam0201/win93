...
class TileShadowNodeHelper extends Group {
  ...
  constructor(tileShadowNode) {
    ...
    if (!tileShadowNode) {
      throw new Error(
        "TileShadowNode instance is required for TileShadowNodeHelper.",
      )
    }
  ...
  init() {
    ...
    for (let i = 0; i < this.tiles.length; i++) {
      ...
      display.material.outputNode = Fn(() => {
        ...
        if (
          !this.tileShadowNode.shadowMap ||
          !this.tileShadowNode.shadowMap.depthTexture
        ) {
      ...
      if (
        this.tileShadowNode._shadowNodes[i] &&
        this.tileShadowNode._shadowNodes[i].shadow
      ) {
        const camHelper = new CameraHelper(
          this.tileShadowNode._shadowNodes[i].shadow.camera,
        )
        camHelper.fog = false
        this.add(camHelper)

...
class ProgressiveLightMap {
  ...
  constructor(renderer, resolution = 1024) {
    ...
    const type = /(Android|iPad|iPhone|iPod)/g.test(navigator.userAgent)
      ? HalfFloatType
      : FloatType
    this._progressiveLightMap1 = new RenderTarget(
      this.resolution,
      this.resolution,
      { type: type },
    )
    this._progressiveLightMap2 = new RenderTarget(
      this.resolution,
      this.resolution,
      { type: type },
    )
    ...
    this._uvMat.outputNode = vec4(
      mix(
        this._previousShadowMap.sample(uv(1)),
        output,
        float(1).div(this._averagingWindow),
      ),
    )
  }
  ...
  addObjectsToLightMap(objects) {
    ...
    for (let ob = 0; ob < objects.length; ob++) {
      ...
      if (object.geometry.hasAttribute("uv") === false) {
        console.warn(
          "THREE.ProgressiveLightMap: All lightmap objects need uvs.",
        )
        continue
      }
      ...
      if (object.geometry.hasAttribute("normal") === false) {
        console.warn(
          "THREE.ProgressiveLightMap: All lightmap objects need normals.",
        )
        continue
      }
    ...
    uv_boxes.forEach((box) => {
      ...
      for (let i = 0; i < uv1.array.length; i += uv1.itemSize) {
        ...
        uv1.array[i + 1] =
          1 - (uv1.array[i + 1] + box.y + padding) / dimensions.h
      }
  ...
  update(camera, blendWindow = 100, blurEdges = true) {
    ...
    for (let l = 0; l < this._lightMapContainers.length; l++) {
      this._lightMapContainers[l].object.oldScene =
        this._lightMapContainers[l].object.parent
    ...
    for (let l = 0; l < this._lightMapContainers.length; l++) {
      ...
      this._lightMapContainers[l].object.material =
        this._lightMapContainers[l].basicMat
      this._lightMapContainers[l].object.oldScene.attach(
        this._lightMapContainers[l].object,
      )
    }
  ...
  showDebugLightmap(visible, position = null) {
    ...
    if (this._labelMesh === null) {
      ...
      labelMaterial.colorNode = texture(
        this._progressiveLightMap1.texture,
      ).sample(uv().flipY())
  ...
  _initializeBlurPlane() {
    ...
    const color = add(
      ...
      this._previousShadowMap.sample(
        uvNode.add(vec2(pixelOffset, pixelOffset)),
      ),
      this._previousShadowMap.sample(
        uvNode.add(vec2(pixelOffset.negate(), pixelOffset)),
      ),
      this._previousShadowMap.sample(
        uvNode.add(vec2(pixelOffset, pixelOffset.negate())),
      ),
      this._previousShadowMap.sample(

...
class SkyMesh extends Mesh {
  constructor() {
    ...
    const vertexNode = /*@__PURE__*/ Fn(() => {
      ...
      const sunIntensity = EE.mul(
        max(
          ...
          float(1.0).sub(
            pow(
              e,
              cutoffAngle.sub(acos(zenithAngleCos)).div(steepness).negate(),
            ),
          ),
        ),
      )
      ...
      const rayleighCoefficient = this.rayleigh.sub(
        float(1.0).mul(float(1.0).sub(vSunfade)),
      )

      // extinction (absorption + out scattering)
      // rayleigh coefficients
      varyingProperty("vec3", "vBetaR").assign(
        totalRayleigh.mul(rayleighCoefficient),
      )
      ...
      varyingProperty("vec3", "vBetaM").assign(
        totalMie.mul(this.mieCoefficient),
      )
    ...
    const fragmentNode = /*@__PURE__*/ Fn(() => {
      ...
      const sunAngularDiameterCos =
        float(0.999956676946448443553574619906976478926848692873900859324)
      ...
      const inv = float(1.0).div(
        pow(
          float(1.0)
            .sub(float(2.0).mul(this.mieDirectionalG).mul(cosTheta))
            .add(g2),
          1.5,
        ),
      )
      ...
      const Lin = pow(
        vSunE
          .mul(add(betaRTheta, betaMTheta).div(add(vBetaR, vBetaM)))
          .mul(sub(1.0, Fex)),
        vec3(1.5),
      )
      Lin.mulAssign(
        mix(
          ...
          clamp(
            pow(sub(1.0, dot(this.upUniform, vSunDirection)), 5.0),
            0.0,
            1.0,
          ),
        ),
      ...
      const retColor = pow(
        texColor,
        vec3(float(1.0).div(float(1.2).add(vSunfade.mul(1.2)))),
      )

...
class PixelationNode extends TempNode {
  ...
  setup() {
    ...
    const sampleNormal = (x, y) =>
      normalNode
        .sample(uvNodeNormal.add(vec2(x, y).mul(this._resolution.zw)))
        .rgb.normalize()
    ...
    const neighborNormalEdgeIndicator = (x, y, depth, normal) => {
      ...
      const depthIndicator = clamp(
        sign(depthDiff.mul(0.25).add(0.0025)),
        0.0,
        1.0,
      )

      return float(1.0)
        .sub(dot(normal, neighborNormal))
        .mul(depthIndicator)
        .mul(normalIndicator)
    }
    ...
    const pixelation = Fn(() => {
      ...
      If(
        this.depthEdgeStrength
          .greaterThan(0.0)
          .or(this.normalEdgeStrength.greaterThan(0.0)),
        () => {

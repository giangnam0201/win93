...
class SobelOperatorNode extends TempNode {
  ...
  setup(/* builder */) {
    ...
    const sobel = Fn(() => {
      ...
      const tx0y0 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(-1, -1)))).xyz,
      )
      const tx0y1 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(-1, 0)))).xyz,
      )
      const tx0y2 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(-1, 1)))).xyz,
      )

      // second column

      const tx1y0 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(0, -1)))).xyz,
      )
      const tx1y1 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(0, 0)))).xyz,
      )
      const tx1y2 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(0, 1)))).xyz,
      )

      // third column

      const tx2y0 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(1, -1)))).xyz,
      )
      const tx2y1 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(1, 0)))).xyz,
      )
      const tx2y2 = luminance(
        sampleTexture(uvNode.add(texel.mul(vec2(1, 1)))).xyz,
      )
...
export const sobel = (node) =>
  nodeObject(new SobelOperatorNode(convertToTexture(node)))

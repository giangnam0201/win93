  ...
  setup(builder) {
    ...
    if (passMRT !== null) {
      ...
      for (const name in passMRT.outputNodes) {
        ...
        if (index >= 0) {
          outputs[name] = texture(this._sampleRenderTarget.textures[index]).mul(
            this.sampleWeight,
          )
        }
      }
      ...
      sampleTexture = texture(this._sampleRenderTarget.texture).mul(
        this.sampleWeight,
      )
    }
...
export const ssaaPass = (scene, camera) =>
  nodeObject(new SSAAPassNode(scene, camera))

...
class DenoiseNode extends TempNode {
  ...
  setup(/* builder */) {
    ...
    const denoiseSample = Fn(([center, viewNormal, viewPosition, sampleUv]) => {
      ...
      const lumaDiff = abs(
        luminance(neighborColor).sub(luminance(center)),
      ).toVar()
      const lumaSimilarity = max(
        float(1.0).sub(lumaDiff.div(this.lumaPhi)),
        0,
      ).toVar()
      const depthDiff = abs(dot(viewPosition.sub(viewPos), viewNormal)).toVar()
      const depthSimilarity = max(
        float(1.0).sub(depthDiff.div(this.depthPhi)),
        0,
      )
    ...
    const denoise = Fn(([uvNode]) => {
      ...
      If(
        depth.greaterThanEqual(1.0).or(dot(viewNormal, viewNormal).equal(0.0)),
        () => {
          return texel
        },
      )
      ...
      const x = sin(
        noiseTexel.element(this.index.mod(4).mul(2).mul(PI)),
      ).toVar()
      const y = cos(
        noiseTexel.element(this.index.mod(4).mul(2).mul(PI)),
      ).toVar()
      ...
      Loop(
        { start: int(0), end: int(16), type: "int", condition: "<" },
        ({ i }) => {
          const sampleDir = this._sampleVectors.element(i).toVar()
          const offset = rotationMatrix
            .mul(
              sampleDir.xy.mul(
                float(1.0).add(sampleDir.z.mul(this.radius.sub(1))),
              ),
            )

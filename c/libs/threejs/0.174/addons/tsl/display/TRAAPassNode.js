...
class TRAAPassNode extends PassNode {
  ...
  updateBefore(frame) {
    ...
    if (velocityOutput !== undefined) {
      ...
      throw new Error(
        "THREE:TRAAPassNode: Missing velocity output in MRT configuration.",
      )
    }
    ...
    if (needsRestart === true) {
      ...
      renderer.copyTextureToTexture(
        this._sampleRenderTarget.texture,
  ...
  setup(builder) {
    ...
    const resolve = Fn(() => {
      ...
      const compressedCurrent = currentColor.mul(
        float(1).div(
          max(max(currentColor.r, currentColor.g), currentColor.b).add(1.0),
        ),
      )
      const compressedHistory = clampedHistoryColor.mul(
        float(1).div(
          max(
            max(clampedHistoryColor.r, clampedHistoryColor.g),
 ...
 */
export const traaPass = (scene, camera) =>
  nodeObject(new TRAAPassNode(scene, camera))

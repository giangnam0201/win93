...
class SMAANode extends TempNode {
  ...
  setup(builder) {
    ...
    const SMAAArea = Fn(([areaTex, dist, e1, e2, offset]) => {
      ...
      texcoord = SMAA_AREATEX_PIXEL_SIZE.mul(texcoord).add(
        float(0.5).mul(SMAA_AREATEX_PIXEL_SIZE),
      )
    ...
    const SMAASearchXLeft = Fn(([edgesTex, searchTex, texcoord, end]) => {
      ...
      coord.x.subAssign(
        this._invSize.x.mul(SMAASearchLength(searchTex, e, 0, 0.5)),
      )
    ...
    const SMAASearchXRight = Fn(([edgesTex, searchTex, texcoord, end]) => {
      ...
      coord.x.addAssign(
        this._invSize.x.mul(SMAASearchLength(searchTex, e, 0.5, 0.5)),
      )
    ...
    const SMAASearchYUp = Fn(([edgesTex, searchTex, texcoord, end]) => {
      ...
      coord.y.subAssign(
        this._invSize.y.mul(SMAASearchLength(searchTex, e.gr, 0, 0.5)),
      )
    ...
    const SMAASearchYDown = Fn(([edgesTex, searchTex, texcoord, end]) => {
      ...
      coord.y.addAssign(
        this._invSize.y.mul(SMAASearchLength(searchTex, e.gr, 0.5, 0.5)),
      )
    ...
    const SMAAWeightsFS = Fn(() => {
      ...
      If(e.g.greaterThan(float(0)), () => {
        ...
        weights.rg = SMAAArea(
          this._areaTextureUniform,
          sqrt_d,
          e1,
          e2,
          float(subsampleIndices.y),
        )
      })
    ...
    this._materialEdges.vertexNode = SMAAEdgeDetectionVS().context(
      builder.getSharedContext(),
    )
    this._materialEdges.fragmentNode = SMAAEdgeDetectionFS().context(
      builder.getSharedContext(),
    )
    this._materialEdges.needsUpdate = true

    this._materialWeights.vertexNode = SMAAWeightsVS().context(
      builder.getSharedContext(),
    )
    this._materialWeights.fragmentNode = SMAAWeightsFS().context(
      builder.getSharedContext(),
    )
    this._materialWeights.needsUpdate = true

    this._materialBlend.vertexNode = SMAABlendVS().context(
      builder.getSharedContext(),
    )
    this._materialBlend.fragmentNode = SMAABlendFS().context(
      builder.getSharedContext(),
    )

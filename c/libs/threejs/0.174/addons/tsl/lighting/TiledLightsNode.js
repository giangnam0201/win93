...
class TiledLightsNode extends LightsNode {
  ...
  getBlock(block = 0) {
    return this._lightIndexes.element(
      this._screenTileIndex.mul(int(2).add(int(block))),
    )
  }
  ...
  setupLights(builder, lightNodes) {
    ...
    Fn(() => {
      Loop(this._tileLightCount, ({ i }) => {
        ...
        const { color, decay, viewPosition, distance } = this.getLightData(
          lightIndex.sub(1),
        )
  ...
  create(width, height) {
    ...
    const lightIndexes = attributeArray(lightIndexesArray, "ivec4").label(
      "lightIndexes",
    )
    ...
    const getTile = (elementIndex) => {
      ...
      return lightIndexes
        .element(tileIndex)
        .element(elementIndex.modInt(stride))
    }

      ...
      for (const filename in zip) {
        ...
        if (filename.endsWith("usd") || filename.endsWith("usda")) {
          if (isCrateFile(zip[filename])) {
            throw Error(
              "THREE.USDZLoader: Crate files (.usdc or binary .usd) are not supported.",
            )
          }
    ...
    function toTriangleIndices(rawIndices, counts) {
      ...
      for (let i = 0; i < counts.length; i++) {
        ...
        if (count === 3) {
          ...
          console.warn(
            "THREE.USDZLoader: Face vertex count of %s unsupported.",
            count,
          )
        }
      }
    ...
    function setTextureParams(map, data_value) {
      ...
      if (data_value["float2 inputs:scale"]) {
        map.repeat = new Vector2().fromArray(
          JSON.parse(
            "[" + data_value["float2 inputs:scale"].replace(/[()]*/g, "") + "]",
          ),
        )
      }
      ...
      if (data_value["float2 inputs:translation"]) {
        map.offset = new Vector2().fromArray(
          JSON.parse(
            "[" +
              data_value["float2 inputs:translation"].replace(/[()]*/g, "") +
              "]",
          ),
        )
      }
        ...
        if (surface !== undefined) {
          if ("color3f inputs:diffuseColor.connect" in surface) {
            ...
            if ('def Shader "Transform2d_diffuse"' in data) {
              setTextureParams(
                material.map,
                data['def Shader "Transform2d_diffuse"'],
              )
            }
          ...
          if ("color3f inputs:emissiveColor.connect" in surface) {
            ...
            if ('def Shader "Transform2d_emissive"' in data) {
              setTextureParams(
                ...
                data['def Shader "Transform2d_emissive"'],
              )
            }
          } else if ("color3f inputs:emissiveColor" in surface) {
            const color = surface["color3f inputs:emissiveColor"].replace(
              /[()]*/g,
              "",
            )
          ...
          if ("normal3f inputs:normal.connect" in surface) {
            ...
            if ('def Shader "Transform2d_normal"' in data) {
              setTextureParams(
                material.normalMap,
                data['def Shader "Transform2d_normal"'],
              )
            }
          }
          ...
          if ("float inputs:roughness.connect" in surface) {
            ...
            if ('def Shader "Transform2d_roughness"' in data) {
              setTextureParams(
                ...
                data['def Shader "Transform2d_roughness"'],
              )
            }
          ...
          if ("float inputs:metallic.connect" in surface) {
            ...
            if ('def Shader "Transform2d_metallic"' in data) {
              setTextureParams(
                ...
                data['def Shader "Transform2d_metallic"'],
              )
            }
          ...
          if ("float inputs:clearcoat.connect" in surface) {
            ...
            if ('def Shader "Transform2d_clearcoat"' in data) {
              setTextureParams(
                ...
                data['def Shader "Transform2d_clearcoat"'],
              )
            }
          ...
          if ("float inputs:occlusion.connect" in surface) {
            ...
            if ('def Shader "Transform2d_occlusion"' in data) {
              setTextureParams(
                material.aoMap,
                data['def Shader "Transform2d_occlusion"'],
              )
            }
          }
        }

...
class EXRExporter {
  async parse(arg1, arg2, arg3) {
    if (
      !arg1 ||
      !(arg1.isWebGLRenderer || arg1.isWebGPURenderer || arg1.isDataTexture)
    ) {
      throw Error(
        "EXRExporter.parse: Unsupported first parameter, expected instance of WebGLRenderer or DataTexture.",
      )
...
function supportedDT(texture) {
  ...
  if (texture.format !== RGBAFormat) {
    throw Error(
      "EXRExporter.parse: Unsupported DataTexture texture format, expected RGBAFormat.",
    )
  }
  ...
  if (
    ...
    texture.image.data.constructor.name !== "Float32Array"
  ) {
    throw Error(
  ...
  if (
    ...
    texture.image.data.constructor.name !== "Uint16Array"
  ) {
    throw Error(
...
async function getPixelData(renderer, rtt, info) {
  ...
  if (renderer.isWebGLRenderer) {
    ...
    dataBuffer = await renderer.readRenderTargetPixelsAsync(
      rtt,
      0,
      0,
      info.width,

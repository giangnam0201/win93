...
export async function requestAudioInput() {
...
const explorePorts = async (compiledPatch) => {
  ...
  for (const [nodeId, val] of Object.entries(
    ...
    if (val.metadata.type === "vradio") {
      ...
      div.style.top = Number(val.metadata.position[1] + 4) + "px" // fix css first child bug ?
      div.classList.add("vradio")
      container.append(div)
    }
...
//
export function sendMsgToWebPd(nodeId, portletId, message) {
...
export function inputRandom(id) {
  /** @type {HTMLInputElement} */
  const inputElement = document.querySelector(`#${id} input[type="range"]`)
  const min = Number.parseFloat(inputElement.min)
  const max = Number.parseFloat(inputElement.max)
  const randomValue = Math.random() * (max - min) + min
  inputElement.value = String(randomValue)
  sendMsgToWebPd(id, ["0"], [randomValue])
}

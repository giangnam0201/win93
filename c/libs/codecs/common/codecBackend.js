...
import { createCodecPlayer } from "./createCodecPlayer.js"

export async function loadWasm(scriptpath) {
  return loadArrayBuffer(scriptpath.replace(".js", ".wasm"))
}

export function initBackend(options) {
let PlayerImpl
let player

const window = {_gPlayerAudioCtx: options?.audioContext}
 ...
 *
 * Version 1.3.9 (check src/README.txt for additional information).
 *
...
/**
 * BaseFileMapper encapsulates "file name mapping" related behavior used in
 * AudioBackendAdapterBase subclasses.
 *
 * Respective file name mapping may be required to deal with "file naming"
 * differences between the various used components, e.g. the Web side code
 * deals with URLs while the legacy code used in the BackandAdapter may
 * expect names compatible with some legacy file system (MS DOS, Amiga DOS, etc).
 *
 * Depending on the specific needs of your BackendAdapter you may need to
 * subclass this default implementation.
 */
class BaseFileMapper {
   ...
   * @returns A filehandle meaningful to the used backend.
   */
   ...
   * @returns "virtual" filename ("real world").
   */
  mapBackendFilename(virtualName) {
    return virtualName
  }
}
/* doc only.. browserify proved to be useless garbage
require('../BaseFileMapper.js');
*/
...
class SimpleFileMapper extends BaseFileMapper {
  ...
  mapFromVirtualFilename(virtualName) {
    const filename = this._mapFs2Uri(virtualName)
  ...
  normalizePathFilename(pathFilenameArray) {
    const path = pathFilenameArray[0]
    const filename = pathFilenameArray[1]

    const tmpPathFilenameArray = new Array(2)
    const p = filename.lastIndexOf("/")
    if (p > 0) {
      tmpPathFilenameArray[0] = path + filename.slice(0, Math.max(0, p))
      tmpPathFilenameArray[1] = filename.slice(Math.max(0, p + 1))
    } else {
  ...
  registerFileData(pathFilenameArray, data) {
    ...
    const normalizedArray = this.normalizePathFilename(pathFilenameArray)
  ...
  _registerEmscriptenFileData(pathFilenameArray, data) {
    ...
    let f
    try {
      if (this.Module.FS_createDataFile === undefined) {
        f = true // backend without FS (ignore for drag&drop files)
      } else {
        f = this.Module.FS_createDataFile(
          pathFilenameArray[0],
          pathFilenameArray[1],
          data,
          true,
 ...
 *
 * Subclass this interface to cover the needs of your specific
 * AudioBackendAdapter implementation.
 */
...
class HEAPF32ScopeProvider extends ScopeDataProvider {
  ...
  getTraceStreams() {
    const result = []
    const n = this.getNumberTraceStreams()

    if (!n) return result

    const ret = this.Module.ccall("emu_get_trace_streams", "number")
    const array = this.Module.HEAP32.subarray(ret >> 2, (ret >> 2) + n)
...
/**
 * Abstract base class that must be subclassed to create concrete AudioBackendAdapter implementations.
 *
...
class AudioBackendAdapterBase {
  constructor(fileMapper, scopeProvider) {
    ...
    this._fileMapper =
      fileMapper === undefined ? new BaseFileMapper() : fileMapper
    this._scopeProvider =
      scopeProvider === undefined
        ? new ScopeDataProvider()
        : scopeProvider
  ...
  _createProducerNode(audioCtx) {
    this.error("_createProducerNode")
  }
  ...
  evalTrackOptions(options) {
    ...
    ScriptNodePlayer.getInstance().setPlaybackTimeout(
      options.timeout === undefined ? -1 : options.timeout * 1000,
    )
   ...
   */
  handleBackendSongAttributes(backendAttr) {
   ...
   *
   * @returns In millis; -1 means no timeout defined.
   */
  getPlaybackTimeout() {
    return -1
  }
  ...
  getMaxTicks() {
    return 1
  } // default: backend already delivers data in tick sized chunks
  getCurrentTick() {
    return 0
  } // default: backend already delivers data in tick sized chunks
  ...
  getMaxPlaybackPosition() {
    return 0
  } // default 0 = seeking not supported; -1 = API not ready
  ...
  remapToFloat(input, len, funcReadFloat, output) {
    alert(
      "error: AudioBackendAdapterBase.remapToFloat - use BufferUtil.remapToFloat() instead",
    )
  }
  // @deprecated use BufferUtil directly
  resampleToFloat(
    ...
    alert(
      "error: AudioBackendAdapterBase.resampleToFloat - use BufferUtil.resampleToFloat() instead",
    )
  }
}
/* doc only.. browserify proved to be useless garbage
require('../AudioBackendAdapterBase.js');
*/
...
class OutputTransformer {
  ...
  resetSampleRate(sampleRate, inputSampleRate) {
    ...
    const bufSize = this._backend.getProcessorBufSize()

    const s =
      Math.round((bufSize * this._sampleRate) / this._inputSampleRate) *
      this._getChannels()
  ...
  genSamples(event) {
    ...
    const timeout = this._getTimeout() // measured in samples

    const isStereoOut = event.outputBuffer.numberOfChannels > 1
    const isStereoIn = this._getChannels() == 2

    const outputL = event.outputBuffer.getChannelData(0)
    const outputR = isStereoOut ? event.outputBuffer.getChannelData(1) : null
    ...
    if (player._isNotPlaying()) {
      ...
      while (this._numberOfSamplesRendered < outSize) {
        if (this._numberOfSamplesToRender === 0) {
          ...
          if (timeout > 0 && this._currentPlaytime > timeout) {
            DbgUtil.trace(
              "'song end' forced after " +
                timeout / ScriptNodePlayer.getWebAudioSampleRate() +
                " secs",
            )
            status = 1
          } else {
          ...
          this._numberOfSamplesToRender = this._updateResampleBuffer(
            this._sourceBuffer,
            this._sourceBufferLen,
            this._sampleRate,
        ...
        if (isStereoOut) {
          ...
          else this._copyMono2Mono(this._resampleBuffer, outputL, outSize)
      }

      if (isStereoOut && isStereoIn) {
        this._applyPanning(player.getPanning(), outputL, outputR, outSize)
      }
   ...
   * @param pan [-1..1]
   */
  _applyPanning(pan, outputL, outputR, outSize) {
    if (pan != null) {
      pan = (pan + 1) * 0.5 // 0..1
  ...
  _increaseCurrentPlaytime(val) {
    const playbackRate = ScriptNodePlayer.getWebAudioSampleRate()
    const multiplier = playbackRate / this._sampleRate
  ...
  _detectSongEndSilence() {
    if (this._silenceTimeout > 0 && this._silenceStarttime > 0) {
      const silenceTime = this._currentPlaytime - this._silenceStarttime
      const silenceTimeout =
        this._silenceTimeout * ScriptNodePlayer.getWebAudioSampleRate()
  ...
  _updateResampleBuffer(input, len, sampleRate, inputSampleRate) {
    const readFunc = this._backend.readFloatSample.bind(this._backend)
    const nChannels = this._getChannels()

    let resampleLen
    if (sampleRate == inputSampleRate) {
      BufferUtil.remapToFloat(
        input,
        len * nChannels,
        readFunc,
        this._resampleBuffer,
      ...
      const bufSize = resampleLen * nChannels

      if (bufSize > this._resampleBuffer.length) {
        this._resampleBuffer = this._allocResampleBuffer(bufSize)
      }

      // only mono and interleaved stereo data is currently implemented..
      BufferUtil.resampleToFloat(
        nChannels,
        0,
        input,
        len,
  ...
  _resampleTickerData() {
    const ticker = this._getTicker()

    // triggers "push" of a batch of add-on data (in sync with respective audio )
    if (ticker !== undefined) {
      ticker.resampleTickerData(
        this._sampleRate,
        this._inputSampleRate,
        this._sourceBufferLen,
        this._backend.getNumberTraceStreams(),
        this._backend.getTraceStreams(),
      )
    }
  }

  _fillEmpty(outSize, outputL, outputR) {
    const availableSpace = outSize - this._numberOfSamplesRendered

    for (let i = 0; i < availableSpace; i++) {
      outputL[i + this._numberOfSamplesRendered] = 0
      if (outputR) {
        outputR[i + this._numberOfSamplesRendered] = 0
      }
    }
    this._numberOfSamplesToRender = 0
  ...
  _copyStereo2Stereo(resampleBuffer, outputL, outputR, outSize) {
    const ticker = this._getTicker()
    const player = ScriptNodePlayer.getInstance()
    const useTicker = ticker !== undefined && !player.isPaused()
    ...
    if (
      ...
      const inputIdx = this._sourceBufferIdx >> 1

      for (let i = 0; i < availableSpace; i++) {
        const j = i + this._numberOfSamplesRendered
  ...
  _copyMono2Mono(resampleBuffer, output, outSize) {
    ...
    if (
      ...
      const availableSpace = outSize - this._numberOfSamplesRendered
      const inputIdx = this._sourceBufferIdx
      for (let i = 0; i < availableSpace; i++) {
        const j = i + this._numberOfSamplesRendered
  ...
  _copyStereo2Mono(resampleBuffer, output, outSize) {
    ...
    if (
      ...
      for (let i = 0; i < availableSpace; i++) {
        const j = i + this._numberOfSamplesRendered
        o =
          (resampleBuffer[this._sourceBufferIdx++] +
            resampleBuffer[this._sourceBufferIdx++]) *
          0.5
        output[j] = o
  ...
  initTicker() {
    ...
    if (ticker !== undefined) {
      ticker.init(
        bufSize,
        this._tickerStepWidth,
        this._backend.readFloatTrace.bind(this._backend),
      )
    }
  ...
  beginTicker() {
    ...
    this._baseTick =
      this._baseTick == null ? 0 : this._baseTick + this._maxTicks
...
/*
 * Abstract AudioBackendAdapter class for use in ScriptProcessorNode context.
 *
...
class ScriptNodeBackendAdapter extends AudioBackendAdapterBase {
  ...
  _createProducerNode(audioCtx) {
    const bufSize = this.getProcessorBufSize()
  ...
  _connectTickerNode(ctx, gainNode) {
    ...
    if (this._externalTicker !== undefined) {
  ...
  setPlaybackTimeout(
    t, // final: DO NOT override or its use in ctor breaks!
  ) {
    const correctSampleRate = ScriptNodePlayer.getWebAudioSampleRate()
  ...
  getPlaybackTimeout() {
    ...
    return this._currentTimeout < 0
      ? -1
      : Math.round((this._currentTimeout / correctSampleRate) * 1000)
  }

  getCurrentPlaytime() {
    return (
      this._transformer.getPlaytime() / ScriptNodePlayer.getWebAudioSampleRate()
    )
  }
...
/* doc only.. browserify proved to be useless garbage
require('./ScriptNodeBackendAdapter.js');
require('../BaseFileMapper.js');
require('../ScopeDataProvider.js');
*/
...
class EmsHEAP16BackendAdapter extends ScriptNodeBackendAdapter {
  ...
  isAdapterReady() {
    if (this.Module.notReady === undefined) return true // default for backward compatibility
  ...
  getAudioBuffer() {
    const ptr = this.Module.ccall("emu_get_audio_buffer", "number") // standard function
    return ptr >> 1 // make it a this.Module.HEAP16 pointer
  }
  getAudioBufferLength() {
    const len = this.Module.ccall("emu_get_audio_buffer_length", "number") // standard function
    return len
  }
  readFloatSample(buffer, idx) {
    return this.Module.HEAP16[buffer + idx] / 0x80_00
  }
  ...
  _decodeBinaryToText(ptr, decoderLabel, len) {
    ...
    for (let i = 0; i < len; i++) {
      // no need for longer texts here
      const t = this.Module.HEAPU8[(ptr + i) >> 0]
      if (t == 0) {
        break
      } else {
        buf.push(t)
...
/* doc only.. browserify proved to be useless garbage
require('./impl/Utils.js');
require('./AbstractTicker2.js');
require('./BaseFileMapper.js');
require('./impl/SimpleFileMapper.js');
require('./ScopeDataProvider.js');
require('./impl/EmscriptenScopeProviders.js');
require('./AudioBackendAdapterBase.js');
require('./impl/OutputTransformer.js');
require('./impl/ScriptNodeBackendAdapter.js');
require('./impl/EmscriptenScriptNodeBackendAdapters.js');
require('./impl/FileCache.js');
*/

/**
 * Generic WebAudio music player.
 *
 * As is still reflected in its legacy name this player was originally (back in ~2012) designed for the
 * use of ScriptProcessorNode based audio generation. Over time the original implementation has been
 * repeatedly updated/redesigned but the APIs mostly should have preserved their backward compatibility
 * with older versions - which accounts for some ugly APIs that would look different if they were redone
 * on a green field today.
 *
 ...
 * panning, etc) have not been implemented yet and more changes are to be expected if these ever were to be
 * added. (The current version of this file is considered to be a work in progress. Unfortunately it seems
 * that the whole AudioWorkletNode infrastructure is still in the green bananas stage today in 2023
 * (contrary to what its proponents might claim..) so there is really no rush to use the "modern"
 * infrastructure (see https://jwothke.wordpress.com/2023/10/12/why-you-should-not-use-audioworklet/ ).
 *
 * The original player code predates the introduction of ES6 and initially the implementation had been based
 * on suggogate class constructs with no means to properly hide implementation details. Eventhough
 * parts of the code have meanwhile been somewhat migrated to ES6, some of the newly available features
 * are not used yet, e.g. "private" / "protected" vars and functions are still only marked using the "_"
 * prefix naming convention but do not use ES6's "private" feature.
 *
 * Have a look at the examples and at the comments in the "public" methods to get an idea for how to
 * use this player.
 *
 *
 * The sequential life-cycle of the player is rather simple:
 *
 * 1) initialize the player with a specific "BackendAdapter"
 * 2) load a music file (that the above "BackendAdapter" knows how to handle)
 * 3) select a specific track (if the loaded music file contains multiple tracks)
 * 4) play/pause the selected track (i.e. start to generate audio data)
 * 5) wait for the current track to end and proceeed with any one of the previous steps.
 *
 *
 * What makes life at this point a wee bit more complicated is the fact that any "file loading" in the
 * Web/browser environment is performed asynchrounously. Respective loading may be needed in any
 * one of the first 4 steps. Oftentimes you may also want to use existing legacy code in steps 2, 3
 * and 4 ("legacy" meaning traditional native program code that expects to use file loading
 * APIs with synchrounous access semantics, e.g CLIB "fopen()").
 *
 * For illustration below some examples where respective asynchronous handling may be relevant:
 *
 * - configuration of the player with a specific BackendAdapter instance: Respective backend code may
 *   consist of a mix of JavaScript and WebAssembly. While the JavaScript is immediately available,
 *   the WebAssembly part is asynchronousy loaded. AudioWorklet based code similarly depends on
 *   "Processor" code that is loaded asynchronously.
 *
 * - BackendAdapter classes may be relying on additional files that first need to be loaded
 *   before the respective adapter logic can even be used, e.g. configuration files that supply
 *   the adapter with default settings (built-in stereo or quality settings, etc).
 *
 * - Loading the actual music file may trigger the need for additional files (e.g. the music file
 *   may rely on additional "audio sample library" files, or the BackendAdapter may need to load
 *   additional "replay" code specific to the music file, etc).
 *
 * - When legacy library code is used in the BackendAdapter (e.g. some emulator logic) the need
 *   to load an additional file may still be detected when the code supposedly already has started
 *   to play music (e.g. webUADE's Amiga emulation).
 *
 *
 * The original player implementation provided APIs for each of the above steps: Wherever such an API
 * may need to load an additional file that API is equipped with some mechanism to resume its
 * functionality asynchronously after the respective file data is actually available (since "Promise"
 * had not been introduced at the time, this was done using callback functions that are passed to
 * respective API calls as parameters - see "on*"). This causes the APIs to look more complicated
 * than one might expect and the recently added Promise based APIs may be more accessible from
 * an end user perspective.
 *
 *
 * The key point to keep in mind is that with regard to "file loading" the player's underlying
 * strategy is based on "load on demand" and "retry after fail": As long as a failure is caused by a
 * file not being synchronously accessible (yet), the player waits until that file has been loaded
 * and then restarts the complete initialization sequence from scratch. The player progresses from
 * one file to the next and typically fails at every step along the way (unless the "preload" feature
 * is specifically used to load certain files in advance). The respective "transient fail" detection
 * is based on the use of the "_fileRequestCallback()" API which must always first be used before any
 * BackendAdapter code tries to synchronously open a file. The respective retry-process occurs
 * behind the scenes and is transparent to the end user.
 */

var ScriptNodePlayer = (function () {
  /**
   * FIXME: this implementation detail should never have been exposed externally.. Instead all the "public"
   * functions should have been published directly on the ScriptNodePlayer level.
   */
  PlayerImpl = function (
    backendAdapter,
    requiredFiles,
  ...
  PlayerImpl.prototype = {
    ...
    _ctor (
      ...
      this._spectrumEnabled =
        spectrumEnabled === undefined ? false : spectrumEnabled
      ...
      const f = this._preloadFiles.bind(this)
      f(
        requiredFiles,
        () => {
    ...
    notify () {
      // used to handle asynchronously initialized backend impls
      if (
        this.deferredPreload !== undefined &&
        this._backendAdapter.isAdapterReady()
      ) {
        // now that the runtime is ready the "_preload" can be started
        const files = this.deferredPreload[0]
        const onCompletionHandler = this.deferredPreload[1]
    ...
    _isAutoPlayCripple () {
      // fixme: meanwhile all browsers have been similarly crippled so this probably has to be adjusted
    ...
    _initByUserGesture () {
      ...
      if (this._bufferSource === undefined) {
        const ctx = window._gPlayerAudioCtx
        ...
        this._tickerNode = this._backendAdapter._connectTickerNode(
          ctx,
          this._gainNode,
        ) // optional add-on
        ...
        } catch {} // ignore for the benefit of Safari(OS X)
      }
    },

    _createBufferSource (ctx) {
    ...
    /**
     * Gets the currently used backend adaopter.
     */
    getBackendAdapter () {
    ...
    setSilenceTimeout (silenceTimeout) {
      ...
      if (
        this._backendAdapter !== undefined &&
        this._backendAdapter.isAdapterReady()
      ) {
        this._backendAdapter.setSilenceTimeout(silenceTimeout)
      } else {
      } // new setting will be used for the next song
    },
    ...
    pause () {
      if (
        !this._isWaitingForFile() &&
        !this._initInProgress &&
        this._isSongReady
      ) {
        this._isPaused = true
      }
     ...
     * not nice to listen to. This "panning" impl allows to "mono"-ify those songs..
     * Only used when WebAudio is creating stereo output.
     *
     * @pan null=inactive; or range; -1 to 1 (-1 is original stereo, 0 creates "mono", 1 is inverted stereo)
     */
    setPanning (pan) {
      if (pan != null) {
        pan = (pan + 1) * 0.5 // 0..1
    ...
    getFreqByteData () {
      if (this._analyzerNode) {
        if (this._freqByteData === 0) {
          this._freqByteData = new Uint8Array(
            this._analyzerNode.frequencyBinCount,
          )
        }
    ...
    /**
     * @returns Default 0 seeking not supported.
     */
    getMaxPlaybackPosition () {
     ...
     */
    getCurrentPlaytime () {
      return this._backendAdapter.getCurrentPlaytime()
    },
     ...
     */
    seekPlaybackPosition (ms) {
      return this._backendAdapter.seekPlaybackPosition(ms)
    },
     ...
     */
    getCached (virtualName) {
      const data = this._getCache().getFile(virtualName)
     ...
     */
    setFileData (virtualNameBackend, data) {
      // data must be Uint8Array
      const virtualName =
        this._backendAdapter.mapBackendFilename(virtualNameBackend)

      const pfn = this._getPathAndFilename(virtualName)
      const fileHandle = this._backendAdapter.registerFileData(pfn, data)
     ...
     *
     * @returns Array with 2 elements: 0: basePath (backend specific - most don't need one),
     *        1: filename (incl. the remainder of the path).
     */
    _getPathAndFilename (virtualName) {
      const sp = virtualName.split("/")
      const fn = sp[sp.length - 1]
      const path = virtualName.slice(0, Math.max(0, virtualName.lastIndexOf("/")))
     ...
     * Testcase: IXS.
     */
    asyncSetFileData (virtualNameBackend, data) {
      // data must be Uint8Array
      ...
      this._initIfNeeded(this.mainVirtualName, this.mainData, this.mainOptions)

      this.mainOnCompletion(virtualNameBackend)
    },

    _isFileNotFound (url, xhr) {
      if (xhr.readyState == 4 && xhr.status >= 400) {
        ...
        try {
          const s = new TextDecoder().decode(
            new Int8Array(xhr.response).subarray(0, 6),
          )
    ...
    loadMusicFromURL (
      ...
      if (onProgress === undefined) onProgress = function () {}
      ...
      const virtualName = this._backendAdapter.mapToVirtualFilename(url)

      this._fileReadyNotify = ""

      if (this._backendAdapter.skipFileLoad()) {
        if (
          this._prepareTrackForPlayback(
            virtualName,
            null,
            options,
            onCompletion,
        ...
        if (!origVol) origVol = undefined // ignore if already muted
        ...
        xhr.onload = function (oEvent) {
          ...
          if (this._isFileNotFound(url, xhr)) {
            ...
            if (origVol !== undefined) this.setVolume(origVol)

            onFail()
          } else {
            DbgUtil.trace(
              "loadMusicFromURL successfully loaded: " + virtualName,
            )
            ...
            if (
              ...
              onCompletion(virtualName)
            } else if (!this._isWaitingForFile()) {
                onFail()
              }
          }
    ...
    _readFile (file) {
      const filename = file.xname === undefined ? file.name : file.xname
      const virtualName = this._backendAdapter.mapToVirtualFilename(filename)

      const regDataFunc = function (virtualName, arrayBuffer) {
        const pfn = this._getPathAndFilename(virtualName)
      ...
      return new Promise((resolve, reject) => {
        if (file.fileBuffer === undefined) {
          const reader = new FileReader()
          reader.onload = () => {
            const fileHandle = regDataFunc(virtualName, reader.result)
            fileHandle === undefined ? reject() : resolve(filename)
          }
          reader.onerror = reject
          reader.readAsArrayBuffer(file)
        } else {
          const fileHandle = regDataFunc(virtualName, file.fileBuffer)
          fileHandle === undefined ? reject() : resolve(filename)
        }
      })
    },
    ...
    _prepareTrackForPlayback (
      ...
      // cache main music file for use in retry attempts FIXME: cleanup status/callback handling
      this.mainVirtualName = virtualName
      this.mainData = data
      this.mainOptions = options
      this.mainOnCompletion = onCompletion
      this.mainOnFail = onFail
    ...
    getDefaultSampleRate () {
      alert(
        "error: getDefaultSampleRate() must be replaced with ScriptNodePlayer.getWebAudioSampleRate()",
      )
    },
    // @deprecated use ScriptNodePlayer.getWebAudioContext() instead
    getAudioContext () {
      alert(
        "error: getAudioContext() must be replaced with ScriptNodePlayer.getWebAudioContext()",
      )
    },
    /*
     ...
     *         false if failure
     */
    _initIfNeeded (virtualName, data, options) {
      const status = this._loadMusicData(virtualName, data, options)
      if (status < 0) {
        this._isSongReady = false
        ...
        const ret = this._backendAdapter.evalTrackOptions(options)
        if (ret !== 0) {
        ...
        if (this.mainVirtualName == virtualName) {
        ...
        this._isPaused = true // fixme: ugly autoplay
        /* </wrapper> */
        return true
      } else {
        this._initInProgress = false
    ...
    _loadMusicDataFromCache (
      ...
      if (data !== undefined) {
        DbgUtil.trace(
          "_loadMusicDataFromCache found cached file using name: " +
            virtualName,
        )
        ...
        if (data == 0) {
          ...
          if (
            ...
            if (this._isWaitingForFile()) {
              // onCompletion(virtualName);	// FIXME: why is this not used here? like in the other places?
            } else {
              onFail()
            }
          }
        ...
        DbgUtil.trace(
          "_loadMusicDataFromCache FAILED to find cached file using name: " +
            virtualName,
        )
    ...
    _iOSHack (ctx) {
      try {
        const source = this._createBufferSource(ctx)

        source.buffer = ctx.createBuffer(1, 1, 22_050) // empty buffer
    ...
    _loadMusicData (virtualName, arrayBuffer, options) {
      ...
      const ret = this._backendAdapter.loadMusicData(
        this._sampleRate,
        pfn[0],
        pfn[1],
        data,
    ...
    _preload (files, id, onCompletionHandler) {
      if (id === 0) {
        // we are done preloading
        onCompletionHandler()
      } else {
        id--
        const funcCompleted = function () {
          this._preload(files, id, onCompletionHandler)
        }.bind(this) // trigger next load

        const virtualName = this._backendAdapter.mapToVirtualFilename(files[id])
        this._preloadFile(virtualName, funcCompleted, true)
      }
    },

    /**
     * Loads a file into the backend so that it is available to be used synchronously.
     *
     * @param  onLoadedHandler callback is invoked for files that currently have a "status unknown"
     *                         once that status changes to "known".
     *
     * @param  notifyOnCached  controls if onLoadedHandler callback should always be called
     *                         (even if status of the file is already known now - as already signaled by
     *                         the return value).
     *
     * @returns  1 file not found error
     *          0 file found and ready for synchronous use
     *         -1 status unknown.
     */
    _preloadFile (virtualName, onLoadedHandler, notifyOnCached) {
      ...
      if (data !== undefined) {
        ...
        if (data == 0) {
          retVal = 1
          DbgUtil.trace(
            "error: _preloadFile could not get cached: " + virtualName,
          )
        } else {
          DbgUtil.trace(
            "_preloadFile found cached file using name: " + virtualName,
          )
          ...
          const pfn = this._getPathAndFilename(virtualName)
          const f = this._backendAdapter.registerFileData(pfn, data)
        }
        if (notifyOnCached) {
        ...
        DbgUtil.trace(
          "_preloadFile FAILED to find cached file using name: " + virtualName,
        )
      ...
      if (!(virtualName in this._getCache().getPendingMap())) {
        // avoid duplicate loading
        this._getCache().getPendingMap()[virtualName] = 1

        const url = this._backendAdapter.mapFromVirtualFilename(virtualName)
        ...
        xhr.onload = function (oEvent) {
          ...
          if (this._isFileNotFound(url, xhr)) {
            ...
            if (arrayBuffer) {
              ...
              const f = this._backendAdapter.registerFileData(pfn, fileData)

              DbgUtil.trace(
                "_preloadFile cached file using name: " + virtualName,
              )
            } else {
              console.log("FATAL ERROR: this should be dead code")
            }
          }

          this._handleSetCacheFile(virtualName, fileData, onLoadedHandler)
        }.bind(this)

        xhr.onreadystatechange = function (oEvent) {
          if (xhr.readyState == 4 && xhr.status >= 400) {
            // triggers for regular/local (e.g. drag&drop) files that do not exist

            // testcase: Sandcastle (SMUS) - the song uses "file not found" as
            // end-marker when looking for instrument files, i.e. it is "normal"
            // for this song to look for non-existing files. But is no instuments at
            // all are found then this is an error and the song fails.

            this._handleSetCacheFile(virtualName, 0, onLoadedHandler)
          }
        }.bind(this)
    ...
    _handleSetCacheFile(virtualName, data, onLoadedHandler) {
      if (data == 0) {
        DbgUtil.trace("failed to load file: " + virtualName)
      }
    ...
    _preloadFiles (files, onCompletionHandler) {
    ...
    _getCache () {
      if (window._fileCache === undefined) {
    ...
    // The below utilities are used to bridge the gap between traditional synchronous
    // "file loading" APIs and the asynchronous model available in the Web environment.
    // They are meant to only be used from BackendAdapter classes and are NOT part of the
    // player's end user interface.

    // The ScriptNodePlayer is designed to automatically handle situations where files are
    // "not ready yet" and its "file loading related" functions report this "problem" with
    // the -1 return code (The player will automatically trigger a "retry" when the file
    // status changes to "is ready".). Whenever a BackendAdapter API tries to load some
    // file it MUST always first call _fileRequestCallback() to "annonce that intention".
    // If that call returns -1 then the BackendAdapter's logic should immediately abort
    // and return that status. Return vulues 0 and 1 mean that the file does not exist
    // or that the file "is ready" and the code can now use synchronous file access
    // operations.

    // A typical usecase is the adaptation of existing legacy code (e.g. some
    // C code that relies on standard C libraries for file loading) for reuse in a
    // BackendAdapter. Ideally the changes to the existing code should be kept to minimum.
    // Tools like EMSCRIPTEN allow to cross- compile existing C/C++ code to JavaScript -
    // including the file system related functionalities. However the generated code
    // still uses the original synchronous file access model, i.e. for it to work all the
    // files must have been put into the file system before the cross-compiled code can
    // use them. The original APIs used in the "legacy" code expect files to either be
    // there or not to be there.. but they know NOTHING about the Web's typical "still
    // looking for the file" limbo. By default the original code will typically fail in
    // respective file loading operations since files have not been previously loaded at
    // the time that the code first tries to access them - the code will have no clue
    // why the file "wasn't there" and just return a regular "file not found" error.

    // The work-around taken here is that the original code has to be manually patched
    // with added _fileRequestCallback() calls in all those places where "file open" type
    // operations is to be used. That call will trigger asynchronous file loading and report
    // whether or not the file is ready to be accessed. If the file is ready, the old code
    // can proceed as originally designed, but if the file is "not ready yet" then that
    // status is now available and it has to be propagated back to the BackendAdapter API
    // that made the call (how the _fileRequestCallback() API can be called depends on
    // used programming language/tool, e.g. see "callback.js/makeEmscripten.bat" in webUADE
    // project for a respective C++/EMSCRIPTEN specific example).

    /*
     * Synchronous callback used by BackendAdapter just before "synchronously" loading a file.
     *
     * Note: The implementation by default is designed for use in EMSCRIPTEN generated code. To use
     * it in other contexts, make sure to define backendAdapter.Module.UTF8ToString() and
     * backendAdapter.mapBackendFilename().
     *
     * @param namePtr   "virtual" backend filename (when used in EMSCRIPTEN context this will be
     *                  an EMSCRIPTEN pointer that first has to be mapped to an actual string.. if
     *                  directly used from JavaScript code this probably is already a string. make
     *                  sure to adapt the above mentioned BackendAdapter functions if necessary)
     */
    _fileRequestCallback (namePtr) {
      if (
        this._backendAdapter.Module === undefined ||
        this._backendAdapter.Module.UTF8ToString === undefined
      ) {
        console.log(
          "error: _backendAdapter.Module.UTF8ToString() MUST be defined in order to use _fileRequestCallback()",
        )
      }

      const virtualNameBackend = this._backendAdapter.Module.UTF8ToString(namePtr)
      const virtualName =
        this._backendAdapter.mapBackendFilename(virtualNameBackend)

      DbgUtil.trace(
        "_fileRequestCallback backend name: " +
          virtualName +
          " > FS name: " +
          virtualName,
      )

      const r = this._preloadFile(
        virtualName,
        () => {
          // file status was previously "pending". trigger reload attempt now the file is here

          const success = this._initIfNeeded(
            this.mainVirtualName,
            this.mainData,
            this.mainOptions,
          )

          if (!success && !this._isWaitingForFile()) {
            this.mainOnFail()
          } else {
            // initIfNeeded already triggered _onTrackReadyToPlay
          }
        },
        false,
      )
    ...
    /*
     * Lets the BackendAdapter directly query the file data - only after the
     * file is actually available in _fileRequestCallback().
     *
     * see _fileRequestCallback() for more information
     */
    _fileDataRequestCallback (namePtr) {
      ...
      return this._getCache().getFile(virtualName)
    },

    /*
     * Lets the BackendAdapter directly query the file size - only after the
     * file is actually available in _fileRequestCallback().
     *
     * see _fileRequestCallback() for more information
     */
    _fileSizeRequestCallback (namePtr) {
      return this._fileDataRequestCallback(namePtr).length
    },
  ...
  return {
    _setGlobalWebAudioCtx () {
      if (window._gPlayerAudioCtx === undefined) {
        // cannot be instantiated 2x (so make it global)
        const errText = "Web Audio API is not supported in this browser"
        try {
          if ("AudioContext" in window) {
            // note: Chrome's baseLatency seems to be 0.01
            window._gPlayerAudioCtx = new AudioContext({
              latencyHint: "playback",
            }) // increases baseLatency to 0.02 - seems to reduce clicking in AudioWorklet scenario
            //				window._gPlayerAudioCtx = new AudioContext();
    ...
    createInstance (
      ...
      if (doOnUpdate !== undefined) {
        console.log(
          "warning: createInstance() no longer uses the 'doOnUpdate' param - backend now has to provide this if necessary!",
        )
      }
      if (bufferSize !== undefined) {
        console.log(
          "warning: createInstance() no longer uses the 'bufferSize' param - backend now has to provide this if necessary!",
        )
      }

      let oldVolume // keep whatever app already used before
     ...
     *
     * FIXME: the instance is an internal implemenation detail that should NEVER have been exposed
     *        in the public API (conceptually all the PlayerImpl functions that are NOT prefixed with "_"
     *        should have been directly exposed here instead).
     */
    getInstance () {
      if (window.player === undefined) {
        if (this.warnedOnce === undefined) {
          console.log(
            "warning: ScriptNodePlayer.getInstance() called while player instance not ready. Check your initialization code.",
          )
    ...
    loadMusicFromURL (url, options, onFail, onProgress) {
      return new Promise((resolve, reject) => {
        const onTrackReadyToPlay = function (filename) {
          const stat = { status: "trackReadyToPlay", file: filename }
          resolve(stat)
        }

        const onCompletion = function () {} // in hindsight this callback is rather useless
    ...
    /**
     * Promise based all-in-one API that allows to directly play a song with a single call -
     * shielding the user from the intermediary asynchonous steps involved under the hood.
     *
     * Beginners may find this easier to use and it may serve as an example for
     * how the underlying/original/older APIs are used. As a trade-off the functionality is
     * hardcoded, and not all the features of the underlying APIs are used.
     *
     * Caution: browsers usually still require a "user engagement" to allow any audio playback,
     * i.e. the below code must be called from a respective UI event handler or the
     * WebAudio may stay muted. (As a workaround pause()/play() may be used to unlock
     * the respective browser garbage at a later time.. Again from a respective
     * event handler.).
     *
     * Only the first 3 parameters are mandatory (optional params may be set to undefined).
     */
    playSong(
      backend,
      url,
      ...
      return new Promise((resolve, reject) => {
        ScriptNodePlayer.initialize(
          ...
          spectrumEnabled,
          externalTicker,
        ) // no use of 'requiredFiles'.. rarely used anyway
          .then(
            (msg) => {
              // resolved
              // map below 2 callbacks to the result of the Promise
              const onTrackReadyToPlay = function (filename) {
                ScriptNodePlayer.getInstance().play() // hard coded autoplay
                resolve({
                  status: "success",
                  ctx: "loadMusicFromURL",
                  file: url,
                })
              }

              const onFail = function (filename) {
                reject({ status: "fail", ctx: "loadMusicFromURL", file: url })
              }
              ...
              ScriptNodePlayer.getInstance().loadMusicFromURL(
                ...
                onProgress,
                onTrackReadyToPlay,
              ) // no use of onCompletion.. rather useless anyway
            },
            (err) => {
              // rejected
              reject({
                status: "fail",
                ctx: "ScriptNodePlayer.initialize",
                file: url,
              })
            },
          )

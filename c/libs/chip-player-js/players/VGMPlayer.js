...
const fileExtensions = ["vgm", "vgz", "gym", "s98", "dro"]

const INT32_MAX = 0x8_00_00_00 // 2147483648

export class VGMPlayer extends Player {
  ...
  loadData(data, filepath) {
    ...
    const meta = {
      title:
        this.core.UTF8ToString(this.core.getValue(metaPtr + 0, "i32")) ||
        filepathMeta.title,
  ...
  processAudioInner(channels) {
    ...
    const samplesWritten = this.core._lvgm_render(
      this.vgmCtx,
      this.buffer,
      this.bufferSize,
    )
  ...
  getVoiceGroups() {
    ...
    for (let i = 0; i < numVoices; i++) {
      const voiceName = this.core.UTF8ToString(
        this.core._lvgm_get_voice_name(this.vgmCtx, i),
      )
      const chipName = this.core.UTF8ToString(
        this.core._lvgm_get_voice_chip_name(this.vgmCtx, i),
      )
  ...
  getVoiceName(index) {
    if (this.vgmCtx) {
      return this.core.UTF8ToString(
        this.core._lvgm_get_voice_name(this.vgmCtx, index),
      )
    }
  }

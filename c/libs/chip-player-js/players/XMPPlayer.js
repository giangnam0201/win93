...
export class XMPPlayer extends Player {
  paramDefs = [
    {
      id: "interpolation",
      label: "Interpolation",
  ...
  processAudioInner(channels) {
    ...
    const err = this.core._xmp_play_buffer(
      this.xmpCtx,
      this.buffer,
      this.bufferSize * 4,
      1,
    ...
    if (err === -1) {
      ...
      throw new Error("xmp_play_buffer failed")
    }
  ...
  _parseMetadata() {
    ...
    const modulePtr = xmp.getValue(infoPtr + 20, "*")
    meta.title = xmp.UTF8ToString(modulePtr, 256)
    meta.system = xmp.UTF8ToString(modulePtr + 64, 256)
    ...
    meta.patterns = xmp.getValue(modulePtr + 128 + 4 * 0, "i32")
    meta.tracks = xmp.getValue(modulePtr + 128 + 4 * 1, "i32")
    meta.numChannels = xmp.getValue(modulePtr + 128 + 4 * 2, "i32") // tracks per pattern
    meta.numInstruments = xmp.getValue(modulePtr + 128 + 4 * 3, "i32")
    meta.numSamples = xmp.getValue(modulePtr + 128 + 4 * 4, "i32")
    meta.initialSpeed = xmp.getValue(modulePtr + 128 + 4 * 5, "i32")
    meta.initialBPM = xmp.getValue(modulePtr + 128 + 4 * 6, "i32")
    meta.moduleLength = xmp.getValue(modulePtr + 128 + 4 * 7, "i32")
    meta.restartPosition = xmp.getValue(modulePtr + 128 + 4 * 8, "i32")
    const instPtr = xmp.getValue(modulePtr + 128 + 4 * 12, "i32")
    ...
    for (let i = 0; i < meta.numInstruments; i++) {
      const ptr = instPtr + i * instStructSize
    ...
    this.metadata = meta
    this.infoTexts = infoText.length > 0 ? [infoText.join("\n\n")] : []
  }
  ...
  loadData(data, filename) {
    ...
    err = this.core._xmp_load_module_from_memory(
      this.xmpCtx,
      dataPtr,
      data.length,
    )
  ...
  isPlaying() {
    const playingState = this.core._xmp_get_player(
      this.xmpCtx,
      XMP_PLAYER_STATE,
    )

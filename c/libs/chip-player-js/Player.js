...
export class Player extends Emitter {
   ...
   * @returns {number} - A pointer to the allocated memory.
   */
  copyToHeap(data) {
  ...
  /**
   * @param {Uint8Array} _data - File contents.
   * @param {string} _filename - Filename for metadata fallback.
   */
  loadData(_data, _filename) {
    throw new Error("Player.loadData() must be implemented.")
  }

  stop() {
    throw new Error("Player.stop() must be implemented.")
  }

  isPlaying() {
    throw new Error("Player.isPlaying() must be implemented.")
  }
  ...
  processAudioInner(_output) {
    throw new Error("Player.processAudioInner() must be implemented.")
  }

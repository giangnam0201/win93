...
let PICNUM = 3; // number of picture walls
let POLYNUM = 6; // number of polyhedra
let OPENNUM = 3; // number of floating opengl's
...
let SX1; let SY1;
let maze;
let theta; let dtheta;

let polypos; let openpos;
let FinX; let FinY;

let eyeX; let eyeY;
let deyeX; let deyeY;

let ratX; let ratY; let ratdX; let ratdY; let rattheta; let ratdtheta;
...
const near = 0.01;
const far = 50;
const fovy = 90;  // Field-of-view in Y direction angle (in degrees)
let aspect;     // Viewport aspect ratio
let eye; let at; let up;

let modelViewMatrix; let projectionMatrix; let scaleMatrix;
let modelViewMatrixLoc; let projectionMatrixLoc; let scaleMatrixLoc;
...
let pointsArray; let colorsArray; let texCoordsArray;
...
function resetVars() {
	...
	for(var i=0; i<PICNUM;i++) {
		const pos = elgible.splice(Math.floor(Math.random() * elgible.length),1)[0];
...
var render = function(){
    ...
    if (!Math.round((FinX+.5-eyeX)*1000)/1000 && !Math.round((FinY+.5-eyeY)*1000)/1000) {
		...
		scaleMatrix=scalem(1,1,height);
	    gl.uniformMatrix4fv( scaleMatrixLoc, false, flatten(scaleMatrix) );
	} else if (Math.round(Math.abs(up[2])*10_000)/10_000-1){
		up = ((Math.round(theta/Math.PI*180 *10_000)/10_000 %360 /90)+5)%2 ? vec3(mult(rotateX(2),vec4(up))) : vec3(mult(rotateY(2),vec4(up)));
	} else {
...
{
	const degtheta=Math.round(theta/Math.PI*180 *10_000)/10_000
 ...
 if ((X+.5)%1 || (Y+.5)%1) {
		...
		const direction = ((degtheta %360 /90)+5 )%4;
		const walls = maze[Y-.5][X-.5];
		// [-Y,+X,+Y,-X]
		// [ 0, 1, 2, 3]
 ...
 if (walls[(direction+1)%4]==1){
			dtheta=Number(Math.PI)/180;
			return [theta+dtheta,X,Y,dtheta,0,0];
		}
...
function newMaze(x, y) { // https://www.dstromberg.com/2013/07/tutorial-random-maze-generation-algorithm-in-javascript/
	...
	while (visited < totalCells) {
		...
		const potential = [[currentCell[0]-1, currentCell[1], 0, 2],	// top

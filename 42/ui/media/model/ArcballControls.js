...
function onWheel(event) {
  if (this.enabled && this.enableZoom) {
    ...
    if (mouseOp != null) {
      ...
      switch (mouseOp) {
        case "ZOOM":
          const wasRotating = this._state === STATE.ANIMATION_ROTATE

          this.updateTbState(STATE.SCALE, !wasRotating)
          ...
          if (this.cursorZoom && this.enablePan) {
            ...
            transformation = this.scale(size, scalePoint)
          } else {
            transformation = this.scale(size, this._gizmos.position)
          }

          if (!wasRotating) {
            this.applyTransformMatrix(transformation)
          }

          if (transformation.camera != null) {
            this._cameraMatrixState.premultiply(transformation.camera)
          }

          if (transformation.gizmos != null) {
            this._gizmoMatrixState.premultiply(transformation.gizmos)
          }

          if (wasRotating) {
            this.applyTransformMatrix(
              this.rotate(this._rotationAxis, this._angleCurrent),
            )
          }
          ...
          this.updateTbState(
            wasRotating ? STATE.ANIMATION_ROTATE : STATE.IDLE,
            false,
          )
